...
| `AgentCBS` | 否 | AGS 托管数据盘，有状态和绑定关系，但通过所属 `VolumeTemplate` 鉴权 |
...
首期同一个 Tool 不允许同时使用 `StorageMounts[]` 和 `VolumeMounts[]`。如果 `CreateSandboxTool` 同时传入两套字段，请求应被拒绝；因此 `StartSandboxInstance.MountOptions[]` 的匹配目标是确定的：旧 Tool 匹配 `StorageMounts[].Name`，新 Tool 匹配 `VolumeMounts[].Name`。
...
| `AgentBucket` | `AgentBucket` | `AccessDomain` | 可选 `SpaceId`；用于声明 Agent Bucket 空间边界 |
...
- 如果存在 `Deleting` / `Error` 状态 AgentCBS，拒绝删除；客户需要等待状态变为可删除状态或联系服务支持处理。
- 删除共享介质类型模板时，只删除 AGS 侧声明，不删除客户底层 COS / CFS / Agent Bucket 资源，也不删除其中的数据。

响应字段：

| 字段 | 说明 |
|------|------|
| `VolumeTemplateId` | 已删除或进入删除流程的模板 ID |

资源不存在时返回资源不存在错误；仍被 Tool 或 AgentCBS 占用时返回资源冲突错误。
  ...
  "AgentCBSSet": [
    {
      ...
      "Chargeable": true,
      "CreateTime": "2026-06-18T10:00:00Z",
      "LastUnbindTime": "2026-06-18T11:00:00Z"
    }
  ]
}
```
...
| 状态 | 含义 | 是否可复用 | 是否可删除 |
|------|------|------------|------------|
| `Creating` | 正在创建数据盘 | 否 | 否 |
| `Available` | 数据盘存在且未绑定实例 | 是 | 是 |
| `Bound` | 已绑定实例 | 否 | 否 |
| `Deleting` | 正在删除 | 否 | 否 |
| `Deleted` | 已删除 | 否 | 否 |
| `Error` | 创建、绑定、解绑或删除失败 | 否 | 否 |
...
- `Deleting` 状态表示已有删除任务在进行中，返回删除中状态，不重复发起删除。
- `Deleted` 或不存在时，返回资源不存在错误。
...
| 字段 | 说明 |
|------|------|
| `AgentCBSId` | 被删除的 AgentCBS ID |
| `Status` | 删除后的状态，通常为 `Deleting` 或 `Deleted` |
...
- 新 Tool 首期不能同时传 `StorageMounts[]` 和 `VolumeMounts[]`；同时传入时请求应被拒绝。
...
| Tool 引用 `VolumeTemplate` | `VolumeTemplate`，需要 `UseVolumeTemplate` 权限 |
